...
export type FinalizeBuyerRealtorReferralConnectionResult =
  | { ok: true }
  | { ok: false; error: string }

/**
 * Completes a buyer–realtor referral connection after the initial notification is created.
 * Call from the follow-up handler when the connection should be committed.
 */
export async function finalizeBuyerRealtorReferralConnection(ctx: {
  buyerId: string
  investorId: string
}): Promise<FinalizeBuyerRealtorReferralConnectionResult> {
  const now = new Date()
  ...
  if (activeLinks.some((link) => link.investorId === ctx.investorId)) {
    return { ok: false, error: "Buyer is already connected to you" }
  }
  ...
  if (connectedToOtherRealtor) {
    return { ok: false, error: "Buyer is already connected to another realtor" }
  }
