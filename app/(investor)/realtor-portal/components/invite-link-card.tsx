...
export function InviteLinkCard() {
  ...
  const referralCode = session?.user?.referralUrl ?? "jamie-alvarez";
  const inviteLink = `parcelhawk.ai/sign-up?ref=${encodeURIComponent(referralCode)}`;
  ...
  return (
    <section className="rounded-xl bg-[#022A43] font-ibm-plex-sans p-4 text-white shadow-sm">
      ...
      <div className="mt-3 flex items-center rounded-lg bg-white/10 p-1 text-xs">
        <span className="truncate px-2 text-blue-50">{inviteLink}</span>
        <button
          type="button"
